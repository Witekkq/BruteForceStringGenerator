...
from string_generator.brute_force_string_generator import (
...
class MyTest(unittest.TestCase):
    ...
    def test_brute_force_direction(self):
        direction_test = BruteForceStringGenerator(
            sequence="aa", direction=Direction.LEFT
        )
        assert next(direction_test) == "ba"
        direction_test = BruteForceStringGenerator(
            sequence="aa", direction=Direction.RIGHT
        )
        assert next(direction_test) == "ab"

    def test_brute_force_edge(self):
        direction_test = BruteForceStringGenerator(
            sequence="za", direction=Direction.LEFT
        )
        assert next(direction_test) == "ab"
        direction_test = BruteForceStringGenerator(
            sequence="az", direction=Direction.RIGHT
        )
        assert next(direction_test) == "ba"

    def test_brute_force_long_string(self):
        long_string = BruteForceStringGenerator(
    ...
    def test_brute_force_with_polish_letters_backward(self):
        custom_string = BruteForceStringGenerator(
            chars="ąćęłńóśźż", min_length=3, direction=Direction.LEFT
        )
        for _ in range(1, 2003):
            next(custom_string)
    ...
    def test_brute_force_with_german_letters_backward(self):
        custom_string = BruteForceStringGenerator(
            chars="äöüß", min_length=3, direction=Direction.LEFT
        )
        for _ in range(1, 2003):
            next(custom_string)
    ...
    def test_brute_force_with_czech_letters_backward(self):
        custom_string = BruteForceStringGenerator(
            chars="ýžáčďéěíňóřšťúů", max_length=4, direction=Direction.LEFT
        )
        for _ in custom_string:
            pass
